...
export interface TunnelRegisterReqMessage extends BaseMessage {
  ...
  localTargetHost?: string; // e.g. "localhost" or "127.0.0.1"
  protocol?: 'http' | 'https';
}
...
export interface HttpRequestChunkMessage extends BaseMessage {
  ...
  requestId: string;
  chunk: string; // Base64 encoded payload chunk
  isBinary?: boolean;
}
...
export interface WsFrameMessage extends BaseMessage {
  ...
  wsId: string;
  isBinary: boolean;
  payload: string; // Base64 encoded if binary or utf8 string
}
...
export type ProtocolMessage =
  ...
  | AuthAckMessage
  | AuthFailMessage
  | TunnelRegisterReqMessage
  | TunnelRegisterAckMessage
  | TunnelRegisterFailMessage
