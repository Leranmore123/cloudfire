...
import { Terminal, Copy, Check, ExternalLink, HelpCircle, Laptop, ArrowRight, Play, Server, Zap } from 'lucide-react';
...
export function QuickStartGuide({ apiKey = 'trk_live_43021d2c8ab8a30c79ed6402964cbb3d1ed62d86464df1b9', defaultPort = '3001' }: QuickStartGuideProps) {
  ...
  const targetDomain = subdomain.trim() ? `${subdomain.trim()}.skyranksolution.com` : 'app.skyranksolution.com';
  const liveUrl = `http://${targetDomain}`;

  const generatedCommand = `node apps/tunnel-agent/ts/dist/cli.mjs tunnel --port ${port || '3001'} --domain ${targetDomain} --edge-ws ws://13.62.54.247/tunnel/connect --api-key ${apiKey}`;
  ...
  return (
    <div className="bg-[#0f172a] border border-[#1e293b] rounded-3xl p-6 sm:p-8 space-y-6 shadow-xl relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#1e293b] pb-6">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xl font-bold text-white">Interactive Local Setup & Help Guide</h3>
          </div>
          <p className="text-xs text-slate-400 mt-1">
            Expose any local project running on your PC (e.g. Next.js, React, Node, Python) to the public internet step-by-step.
          </p>
        </div>
        <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1.5 rounded-full">
          <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse"></span>
          <span className="text-xs font-semibold text-emerald-400">Edge Gateway Ready</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Step 2: Configure Local Target Port */}
        <div className="bg-[#090d16] border border-[#1e293b] rounded-2xl p-5 space-y-3 relative group hover:border-indigo-500/30 transition-colors">
          ...
          <h4 className="text-sm font-semibold text-white">Enter Your Local Port & Subdomain</h4>
      ...
      <div className="bg-[#090d16] border border-[#1e293b] rounded-2xl p-6 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-sm font-semibold text-white">Copy & Execute Terminal Command</h4>
          </div>
          <button
            onClick={handleCopyCmd}
            className="flex items-center gap-1.5 bg-sky-500 hover:bg-sky-400 text-slate-950 font-semibold px-4 py-1.5 rounded-xl text-xs transition-colors"
          >
            {copiedCmd ? (
              <>
